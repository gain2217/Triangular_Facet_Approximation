...
options = optimoptions('lsqnonlin', 'Algorithm','levenberg-marquardt', 'Display','final',...
    'MaxFunEvals',1e4, 'MaxIter',1e3, 'TolFun',1e-6, 'TolX',1e-6);
...
[paras_init,resnorm_init,residual_init,exitflag_init,output_init] = lsqnonlin(...
    @(p)residual_KRt_fisheye_robust(double(X1), double(X2), imsize1, imsize2, p, sigma_F, sigma_H, lambda), [1000 1000 0 0 0 0 0],...
    lb,ub,options);
...
[paras,resnorm,residual,exitflag,output] = lsqnonlin(...
    @(p)residual_KRt_fisheye_robust(double(X1_ok), double(X2_ok), imsize1, imsize2, p, sigma_F, sigma_H, lambda), paras_init,...
    lb,ub,options);
k1 = paras(1);
k2 = paras(2);
theta = paras(3:5);
